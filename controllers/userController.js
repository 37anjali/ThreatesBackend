   ...
   try{
    ...
    if(existingData){
        ...
        if(comparePassword){
            ...
            return res.json({msg:"user login successfuly",success:true,token:token})
        }else{
...
const deleteUser = async(req,res)=>{
    ...
    // console.log("userLogged id =",req.user._id)
    if(req.user._id!== id){
...
async function sendResetEmail(email,token) {
    const transporter = nodemailer.createTransport({
        ...
        auth: {
          user: "anjali.anchaurasiya210@gmail.com",
          pass: "gprf pqfi dkio kwsi",
        },
      }); 
